    ...
    def setHeuristic(self, typePreference, node_destination,city):
        ...
        if typePreference == 1:
            ...
            if self.station.name != node_destination.station.name:
                d = minimumDistance(self, node_destination)
                v = city.max_velocity
                self.h = d/v
            else:
                self.h = 0
        ...
        # Minimum Distance
            ...
            if self.station.name != node_destination.station.name:
                # Distance
                self.h = minimumDistance(self, node_destination)
            else:
                self.h = 0
...
def Expand(fatherNode, stationList, typePreference, node_destination, costTable,city):
    ...
    for station in fatherNode.station.destinationDic:
        ...
        newNode.time = costTable[fatherNode.station.id][station] + fatherNode.time
        newNode.walk = costTable[fatherNode.station.id][station] + fatherNode.walk
        newNode.transfers = costTable[fatherNode.station.id][station] + fatherNode.transfers
        newNode.num_stopStation = costTable[fatherNode.station.id][station] + fatherNode.num_stopStation
...
def minimumDistance(origin, destination):
    ...
    """
    x = destination.station.x - origin.station.x
    y = destination.station.y - origin.station.y
    # Distance
    return sqrt(x**2 + y**2)
